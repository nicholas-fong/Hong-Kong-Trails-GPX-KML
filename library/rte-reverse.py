...
def reverse_gpx_route(input_path, output_path, strip_time=True):
    ...
    # Find all routes (handles namespaces dynamically)
    for rte in root.iter():
        if rte.tag.endswith('rte'):
            # Extract all route points in this route
            points = [child for child in rte if child.tag.endswith('rtept')]
            
            # Remove the original points from the route
            for pt in points:
                rte.remove(pt)
            
            # Strip timestamps if requested (though route points rarely have them, it keeps it safe)
            if strip_time:
                for pt in points:
    ...
    print(f"Done! Reversed route saved to: {output_path}")
...
if __name__ == "__main__":
    ...
    # 3. Warning: If output file exists, issue a warning and stop
        ...
        try:
            reverse_gpx_route(input_file, output_file)
